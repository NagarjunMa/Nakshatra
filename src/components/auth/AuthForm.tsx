...
const COPY = {
  ...
  signup: {
    ...
    body: "Create your account now. You can build and preview your portfolio before publishing.",
    googleAction: "Continue with Google",
...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  async function resendSignupCode() {
    ...
    setError("");
    const { ok, body } = await startAuthentication({
        ...
        <section className="account-panel">
          {screen === "credentials" && (
            <>
              ...
              <h1 className="account-title">{copy.title}</h1>
              ...
              <form onSubmit={handleCredentials} className="account-form">
                ...
                />
                <label className="account-field" htmlFor={`${mode}-password`}>
                  <span>Password</span>
                ...
                {mode === "signup" && <p className="account-field-help">Use at least 8 characters.</p>}
                {mode === "login" && (
                  <button
                    type="button"
          ...
          {screen === "verify" && (
            <form onSubmit={handleVerify} className="account-sent">
              ...
              <div className="account-sent-actions">
                <button type="submit" className="auth-primary-btn" disabled={pendingAction !== null || otp.length !== 6}>
                ...
                <button type="button" className="account-change-email" onClick={() => { setError(""); setScreen("credentials"); }}>
                  Change email
                </button>
              </div>
            </form>
          ...
          {screen === "recovery" && (
            <form onSubmit={handleRecovery} className="account-sent">
              ...
              <div className="account-sent-actions">
                <button type="submit" className="auth-primary-btn" disabled={pendingAction !== null}>
                  {pendingAction === "recovery" ? "Sending..." : "Send recovery email"}
                </button>
                <button type="button" className="account-change-email" onClick={() => { setError(""); setScreen("credentials"); }}>Back to sign in</button>
              </div>
            </form>
          )}
