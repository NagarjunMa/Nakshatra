...
- `npm test` runs Vitest with V8 coverage, enforces the configured whole-application baseline, and then fails when an individual feature mapper, service, or contract falls below 80% for statements, branches, functions, or lines.
- `npm run coverage:check:features` validates the generated `coverage/coverage-summary.json`; it is part of `npm test` and must not be treated as an optional reporting step.
